...
BEFORE INSERT ON deanery.subject
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON deanery.schedule
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON deanery.student
FOR EACH ROW
BEGIN
...
BEFORE INSERT ON deanery.teacher
FOR EACH ROW
BEGIN
	SET new.phone = REGEXP_REPLACE(new.phone, '[^0-9]+', '');
END//
...
BEFORE UPDATE ON deanery.subject
FOR EACH ROW
BEGIN
IF NEW.lecture_hours < 0 THEN
	SET NEW.lecture_hours = 0;
...
BEFORE UPDATE ON deanery.student
FOR EACH ROW
BEGIN
	SET new.phone = REGEXP_REPLACE(new.phone, '[^0-9]+', '');
END//
...
BEFORE UPDATE ON deanery.teacher
FOR EACH ROW
BEGIN
	SET new.phone = REGEXP_REPLACE(new.phone, '[^0-9]+', '');
END//
...
BEFORE UPDATE ON deanery.schedule
FOR EACH ROW
BEGIN
IF NEW.week_type != "Щотижня" AND NEW.week_type != "Знаменник" AND NEW.week_type != "Чисельник" THEN
	SET NEW.week_type = "Щотижня";
...
BEGIN
    SELECT COUNT(*) INTO subjects FROM deanery.subject;
END//

CREATE PROCEDURE count_students ( OUT students INT)
BEGIN
    SELECT COUNT(*) INTO students FROM deanery.student;
END//

CREATE PROCEDURE count_teachers ( OUT teachers INT)
BEGIN
    SELECT COUNT(*) INTO teachers FROM deanery.teacher;
END//
